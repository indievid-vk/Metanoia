  ...
  return (
    <div className="pb-12 px-4 max-w-4xl mx-auto">
      <div className="text-center mb-8">
        ...
        <div className="mt-6 flex flex-col items-center gap-2 font-izhitsa text-lg text-[var(--color-ink)]/90 bg-white/40 p-4 rounded-lg border border-[var(--color-cinnabar)]/10 shadow-sm">
          <p className="capitalize font-izhitsa text-2xl text-[var(--color-cinnabar)]">
          ...
          <div className="space-y-1 text-sm sm:text-base">
            <p>
              ...
              <span>{currentDate.toLocaleDateString('ru-RU', { year: 'numeric', month: 'long', day: 'numeric' })}</span>
            </p>
            <p>
              <span className="text-[var(--color-ink)]/50 uppercase tracking-widest mr-2">Старый стиль:</span>
              <span>{oldStyleDate.toLocaleDateString('ru-RU', { year: 'numeric', month: 'long', day: 'numeric' })}</span>
            </p>
          </div>
        </div>
      ...
      <div className="space-y-8">
        ...
        <div className="bg-white/50 rounded-xl p-6 shadow-sm border border-[var(--color-ink)]/10">
          ...
          <div className="space-y-4 font-izhitsa text-[var(--color-ink)]">
            {data.fasting.round_week && (
              <div className="mb-4">
                <p dangerouslySetInnerHTML={{ __html: data.fasting.round_week }} className="prose prose-sm prose-a:text-[var(--color-cinnabar)] prose-a:no-underline hover:prose-a:underline text-xl font-izhitsa leading-relaxed text-[var(--color-cinnabar)]/90" />
              </div>
            )}
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {data.fasting.voice && (
                <div className="bg-white/40 p-3 rounded border border-[var(--color-ink)]/5">
                   ...
                   <span className="font-izhitsa text-2xl text-[var(--color-cinnabar)]">{data.fasting.voice}</span>
                </div>
              )}
                ...
                {data.fasting.fasting || data.fasting.description ? (
                  <div className="flex-1">
                    {data.fasting.fasting && (
                      <p dangerouslySetInnerHTML={{ __html: data.fasting.fasting }} className="text-lg font-izhitsa text-[var(--color-ink)] leading-tight" />
                    )}
                  </div>
                ) : (
                  <p className="text-lg font-izhitsa text-[var(--color-ink)]">Поста нет</p>
                )}
              </div>
            </div>
          ...
          <div className="bg-white/50 rounded-xl p-6 shadow-sm border border-[var(--color-ink)]/10">
            ...
            <ul className="space-y-3">
              {data.holidays.map((holiday, i) => (
                <li key={i} className="font-izhitsa text-[var(--color-ink)] text-lg border-l-2 border-[var(--color-cinnabar)]/20 pl-4 py-1 leading-tight">{holiday.title}</li>
              ))}
            </ul>
          </div>
        )}
          ...
          <div className="bg-white/50 rounded-xl p-6 shadow-sm border border-[var(--color-ink)]/10">
            ...
            <div className="flex flex-wrap gap-2">
              {data.saints.map((saint, i) => (
                <span key={i} className="bg-white/40 border border-[var(--color-ink)]/10 px-3 py-1.5 rounded-lg font-izhitsa text-[var(--color-ink)]/90 text-sm hover:bg-white transition-colors">{saint.title}</span>
              ))}
            </div>
          </div>
        )}
          ...
          <div className="bg-white/50 rounded-xl p-6 shadow-sm border border-[var(--color-ink)]/10">
            ...
            <ul className="space-y-2">
              {data.ikons.map((ikon, i) => (
                <li key={i} className="font-izhitsa text-[var(--color-ink)]/90 italic border-b border-[var(--color-ink)]/5 pb-2 last:border-0">{ikon.title || ikon.clean_title}</li>
              ))}
            </ul>
          </div>
        )}
        ...
        <div className="bg-white/50 rounded-xl p-6 shadow-sm border border-[var(--color-ink)]/10">
          <div className="flex items-center gap-3 mb-4 border-b border-[var(--color-cinnabar)]/10 pb-2">
            ...
            <h3 className="font-izhitsa text-xl text-[var(--color-cinnabar)]">Богослужебные чтения</h3>
          </div>
            ...
            <div className="space-y-6 mb-8">
              {data.texts.map((textObj, i) => (
                <div key={i} className="relative bg-white/30 p-4 rounded-lg border border-[var(--color-ink)]/5">
                  <div className="font-izhitsa text-[var(--color-ink)]/90 text-lg leading-relaxed azbyka-content"
        ...
        <div className="bg-white/60 rounded-xl p-6 shadow-sm border border-[var(--color-ink)]/10">
          <div className="flex items-center gap-3 mb-4">
            <Info className="text-[var(--color-cinnabar)]" size={24} />
            <h3 className="font-izhitsa text-xl text-[var(--color-ink)]">Календарь постов и трапез</h3>
          </div>

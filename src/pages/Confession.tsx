...
export default function Confession() {
  ...
  const selectedSinsByPassion = PASSIONS.map(passion => ({
    passion,
    sins: selectedSinsList.filter(s => s.passion === passion)
  })).filter(group => group.sins.length > 0);
    ...
    <div className="max-w-4xl mx-auto space-y-8 pb-12">
      <div className="pb-8">
        ...
        <div className="text-center mb-8 relative">
          <h1 className="font-izhitsa text-3xl sm:text-5xl text-[var(--color-cinnabar)] mb-2">
            Дневник кающегося
          </h1>
          ...
          <button 
            ...
            className="inline-flex items-center gap-2 px-8 py-3 bg-[var(--color-cinnabar)] text-[var(--color-parchment)] rounded-full font-izhitsa text-lg shadow-lg hover:shadow-xl hover:brightness-110 transition-all active:scale-95 mb-6"
          >
            Памятка
          </button>

          <p className="font-izhitsa text-[var(--color-ink)]/80 max-w-2xl mx-auto px-4">
        ...
        <div className="mb-8 bg-white/40 p-4 rounded-lg shadow-inner border border-[var(--color-ink)]/10">
          <div className="flex justify-between items-center mb-4 border-b border-[var(--color-cinnabar)]/20 pb-2">
            <h2 className="font-izhitsa text-2xl text-[var(--color-cinnabar)]">На исповеди</h2>
            <div className="flex items-center gap-3">
              <span className="text-sm text-[var(--color-ink)]/60">Всего: {selectedSinsList.length}</span>
              {selectedSinsByPassion.length > 0 && (
                <>
                  {expandedReportPassions.length === 0 ? (
                    <button 
                      onClick={() => setExpandedReportPassions(selectedSinsByPassion.map(g => g.passion))}
                      className="text-xs text-[var(--color-cinnabar)] underline opacity-70 hover:opacity-100"
                    >
                      Развернуть все
                    </button>
                  ) : (
                    <button 
                      onClick={() => setExpandedReportPassions([])}
                      className="text-xs text-[var(--color-cinnabar)] underline opacity-70 hover:opacity-100"
                    >
                      Свернуть все
                    </button>
          ...
          <div className="space-y-4 mb-4">
            {selectedSinsByPassion.map(({ passion, sins }) => {
              const isOpen = expandedReportPassions.includes(passion);
              return (
                <div key={passion} id={`report-passion-${passion}`} className="scroll-mt-4">
                  <button 
                    onClick={() => {
                      const next = isOpen 
                        ? expandedReportPassions.filter(p => p !== passion)
                        : [...expandedReportPassions, passion];
                      setExpandedReportPassions(next);
                      if (!isOpen) {
                        setTimeout(() => {
                          document.getElementById(`report-passion-${passion}`)?.scrollIntoView({ behavior: 'smooth', block: 'start' });
                        }, 100);
                      }
                    }}
                    className={`w-full flex items-center justify-between p-3 rounded-lg border transition-all ${
                      isOpen 
                        ? 'bg-[var(--color-cinnabar)]/5 border-[var(--color-cinnabar)]/30' 
                        : 'bg-white/60 border-[var(--color-ink)]/10 hover:bg-white/80'
                    }`}
                  >
                    <span className="font-izhitsa text-lg text-[var(--color-ink)]">{passion}</span>
                    <div className="flex items-center gap-2">
                      <span className="text-xs bg-[var(--color-cinnabar)]/10 text-[var(--color-cinnabar)] px-2 py-0.5 rounded-full">{sins.length}</span>
                      {isOpen ? <ChevronUp size={18} className="text-[var(--color-cinnabar)]" /> : <ChevronDown size={18} className="text-[var(--color-ink)]/40" />}
                    </div>
                  </button>
                  
                  {isOpen && (
                    <ul className="mt-2 space-y-2 pl-2 border-l-2 border-[var(--color-cinnabar)]/20 ml-2 animate-in fade-in slide-in-from-top-1 duration-200">
                      {sins.map(sin => {
                        ...
                        return (
                          <li key={sin.id} className="flex items-start bg-white/80 p-2 rounded border border-[var(--color-ink)]/5 shadow-sm">
                            ...
                            <div className="flex-1 cursor-pointer" onClick={() => setExpandedSelectedSin(isExpanded ? null : sin.id)}>
                              ...
                              {isExpanded && (
                                <div className="mt-2 text-sm text-[var(--color-ink)]/80 border-t border-[var(--color-ink)]/5 pt-2">
                                  {sin.description && <p className="mb-2 italic opacity-80">{sin.description}</p>}
                                  {sinNotes[sin.id] && (
                                    <div className="bg-[var(--color-parchment)]/50 p-2 rounded border border-[var(--color-ink)]/10">
                                      <span className="text-[10px] uppercase tracking-wide opacity-60 block mb-1">Личные обстоятельства:</span>
                                      <p className="italic">{sinNotes[sin.id]}</p>
                                    </div>
                                  )}
                                  {!sin.description && !sinNotes[sin.id] && (
                                    <p className="italic opacity-60 text-xs">Нет доп. описания</p>
                                  )}
                                </div>
                              )}
                            </div>
          ...
          <button 
            ...
            className="w-full py-4 bg-[var(--color-parchment)] border border-[var(--color-cinnabar)] text-[var(--color-cinnabar)] rounded-lg font-izhitsa text-xl shadow-sm hover:shadow-md transition-shadow"
          >
            Исповедано
          </button>
        </div>
      ...
      <div className="space-y-4">
        {PASSIONS.map(passion => {
          ...
          return (
            <div key={passion} id={`passion-${passion}`} className="border border-[var(--color-ink)]/20 rounded-lg overflow-hidden bg-white/30 scroll-mt-4">
              <button 
                onClick={() => {
                  const next = isExpanded ? null : passion;
                  setExpandedPassion(next);
                  if (next) {
                    setTimeout(() => {
                      document.getElementById(`passion-${passion}`)?.scrollIntoView({ behavior: 'smooth', block: 'start' });
                    }, 100);
                  }
                }}
                className="w-full flex items-center justify-between p-4 bg-[var(--color-parchment)] hover:bg-black/5 transition-colors"
              >
                <h3 className="font-izhitsa text-xl text-[var(--color-ink)]">{passion}</h3>
              ...
              {isExpanded && (
                <div className="p-2 space-y-2 bg-[var(--color-parchment)]/50">
                        ...
                        <div className="flex items-start p-3">
                          ...
                          <div className="flex-1 cursor-pointer" onClick={() => setExpandedSin(isSinExpanded ? null : sin.id)}>
                            <div className="flex justify-between items-start">
                              <h4 className={`font-izhitsa text-lg ${isSelected ? 'text-[var(--color-cinnabar)]' : 'text-[var(--color-ink)]'}`}>
                                {sin.title}
                              </h4>
                            ...
                            {isSinExpanded && (
                              <div className="mt-3 text-sm text-[var(--color-ink)]/80 leading-relaxed">
                                ...
                                <div className={`pt-3 border-t border-[var(--color-ink)]/10 ${sin.description ? 'mt-4' : 'mt-2'}`}>
                                  <label className="block text-xs text-[var(--color-ink)]/60 mb-1 uppercase tracking-wide">
                  ...
                  <button 
                    ...
                    className="w-full py-3 mt-2 border-2 border-dashed border-[var(--color-ink)]/20 rounded-md text-[var(--color-ink)]/60 hover:text-[var(--color-cinnabar)] hover:border-[var(--color-cinnabar)]/40 transition-colors flex items-center justify-center text-sm font-izhitsa"
                  >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-[var(--color-parchment)] w-full max-w-sm rounded-xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="bg-[var(--color-cinnabar)] text-[var(--color-parchment)] p-4">
              <h3 className="font-izhitsa text-xl">Добавить грех</h3>
              <p className="text-xs opacity-80">Страсть: {customSinPassion}</p>
            </div>
            
            <form onSubmit={handleAddCustomSin} className="p-4 space-y-4">
              <div>
                <label className="block text-sm text-[var(--color-ink)]/80 mb-1">Название греха</label>
                <input 
                  type="text" 
                  required
              ...
              <div>
                <label className="block text-sm text-[var(--color-ink)]/80 mb-1">Описание (необязательно)</label>
                <textarea 
                  value={customSinDesc}
              ...
              <div>
                <label className="block text-sm text-[var(--color-ink)]/80 mb-1">Тяжесть</label>
                <select 
              ...
              <div className="flex space-x-3 pt-4 border-t border-[var(--color-ink)]/10">
                ...
                <button 
                  type="submit"
                  className="flex-1 py-2 bg-[var(--color-cinnabar)] text-[var(--color-parchment)] rounded hover:bg-[var(--color-cinnabar)]/90 transition-colors"
                >
                  Добавить
                </button>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => setIsMemoOpen(false)}>
          ...
          >
            <div className="sticky top-0 bg-[var(--color-cinnabar)] text-[var(--color-parchment)] p-4 flex justify-between items-center z-10 shadow-md">
              <h3 className="font-izhitsa text-2xl">Грехи, страсти, добродетели</h3>
              <button onClick={() => setIsMemoOpen(false)} className="p-1 hover:bg-white/20 rounded-full transition-colors">
                <X size={24} />
              </button>
            </div>
            
            <div className="p-6 space-y-6" id="memo-scroll-container">
              <div id="memo-toc" className="bg-white/50 p-5 rounded-lg border border-[var(--color-cinnabar)]/20 shadow-md">
                <h4 className="font-izhitsa text-xl text-[var(--color-cinnabar)] mb-3 border-b border-[var(--color-cinnabar)]/10 pb-1 text-center italic">Содержание</h4>
                <ul className="list-decimal list-inside space-y-2 text-base text-[var(--color-ink)] font-izhitsa">
                  <li><button onClick={(e) => { e.preventDefault(); const target = document.getElementById('razvitie-greha'); target && target.scrollIntoView({ behavior: 'smooth', block: 'start', inline: 'nearest' }); }} className="hover:underline text-left">Развитие греха</button></li>
                  <li><button onClick={(e) => { e.preventDefault(); const target = document.getElementById('stepeni-tyazhesti'); target && target.scrollIntoView({ behavior: 'smooth', block: 'start', inline: 'nearest' }); }} className="hover:underline text-left">Степени тяжести грехов</button></li>
                  <li><button onClick={(e) => { e.preventDefault(); const target = document.getElementById('dobrodeteli-protiv-strastey'); target && target.scrollIntoView({ behavior: 'smooth', block: 'start', inline: 'nearest' }); }} className="hover:underline text-left">Добродетели против страстей</button></li>
                </ul>
              </div>
              ...
              <div className="space-y-4 text-[var(--color-ink)]">
                <h4 id="razvitie-greha" className="font-izhitsa text-xl text-[var(--color-cinnabar)] border-b border-[var(--color-cinnabar)]/20 pb-2 scroll-mt-20">Как развивается грех</h4>
                
                <div className="space-y-6 text-sm md:text-base">
                  {/* Стадия 1 */}
                  <div className="bg-white/50 p-4 rounded-lg border border-[var(--color-ink)]/10">
                    <h5 className="font-izhitsa text-lg text-[var(--color-cinnabar)] mb-3 border-b border-[var(--color-cinnabar)]/10 pb-1">Стадия 1 — Прилог</h5>
                  ...
                  {/* Стадия 2 */}
                  <div className="bg-white/50 p-4 rounded-lg border border-[var(--color-ink)]/10">
                    <h5 className="font-izhitsa text-lg text-[var(--color-cinnabar)] mb-3 border-b border-[var(--color-cinnabar)]/10 pb-1">Стадия 2 — Сочетание</h5>
                    <ul className="space-y-2">
                  ...
                  {/* Стадия 3 */}
                  <div className="bg-white/50 p-4 rounded-lg border border-[var(--color-ink)]/10">
                    <h5 className="font-izhitsa text-lg text-[var(--color-cinnabar)] mb-3 border-b border-[var(--color-cinnabar)]/10 pb-1">Стадия 3 — Сосложение</h5>
                    <ul className="space-y-2">
                  ...
                  {/* Стадия 4 */}
                  <div className="bg-white/50 p-4 rounded-lg border border-[var(--color-ink)]/10">
                    <h5 className="font-izhitsa text-lg text-[var(--color-cinnabar)] mb-3 border-b border-[var(--color-cinnabar)]/10 pb-1">Стадия 4 — Совершение греха</h5>
                    <ul className="space-y-2">
                ...
                <h4 id="stepeni-tyazhesti" className="font-izhitsa text-xl text-[var(--color-cinnabar)] border-b border-[var(--color-cinnabar)]/20 pb-2 mt-8 scroll-mt-20">Степени тяжести грехов</h4>
                <div className="space-y-4 text-sm md:text-base">
                  ...
                  <p className="text-red-800 bg-red-50 p-3 rounded border border-red-200">
                  ...
                  <div className="bg-white/50 p-4 rounded-lg border border-red-200">
                    <h5 className="font-izhitsa text-lg text-red-700 mb-2">Смертные грехи</h5>
                    ...
                    <div className="mt-3 text-sm bg-red-50 p-2 rounded border border-red-100">
                      <p className="text-red-800">Что делать:</p>
                  ...
                  <div className="bg-white/50 p-4 rounded-lg border border-orange-200">
                    <h5 className="font-izhitsa text-lg text-orange-700 mb-2">Тяжкие грехи</h5>
                    ...
                    <div className="mt-3 text-sm bg-orange-50 p-2 rounded border border-orange-100">
                      <p className="text-orange-800">Что делать:</p>
                  ...
                  <div className="bg-white/50 p-4 rounded-lg border border-green-200">
                    <h5 className="font-izhitsa text-lg text-green-700 mb-2">Простительные грехи</h5>
                    ...
                    <div className="mt-3 text-sm bg-green-50 p-2 rounded border border-green-100">
                      <p className="text-green-800">Что делать:</p>
                ...
                <h4 id="dobrodeteli-protiv-strastey" className="font-izhitsa text-xl text-[var(--color-cinnabar)] border-b border-[var(--color-cinnabar)]/20 pb-2 mt-8 scroll-mt-20">Добродетели против страстей</h4>
                ...
                <div className="space-y-3">
                  {[
                    ...
                    <div key={item.passion} className="bg-white/50 p-4 rounded-lg border border-[var(--color-ink)]/10">
                      <p className="text-[var(--color-ink)] text-lg">{item.passion}</p>
                      <p className="text-sm text-[var(--color-ink)]/80 mb-2">{item.description}</p>
                      <p className="text-xs text-[var(--color-ink)]/60 mb-2"><span className="opacity-80">Примеры:</span> {item.examples}</p>
                      <div className="border-t border-[var(--color-ink)]/10 pt-2 mt-2">
                        <p className="text-[var(--color-cinnabar)]">{item.virtue}</p>
                        <p className="text-sm text-[var(--color-ink)]/80">{item.vDescription}</p>
                        <p className="text-xs text-[var(--color-ink)]/60"><span className="opacity-80">Примеры:</span> {item.vExamples}</p>
                      </div>

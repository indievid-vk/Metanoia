    ...
    <div className="pb-24 max-w-2xl mx-auto px-4">
      ...
      {!hasSins ? (
        <div className="text-center py-12 opacity-60">
          ...
          <p className="font-izhitsa text-lg">Ваш список пуст.</p>
        ...
        <div className="space-y-8">
          <div className="bg-white/40 p-6 rounded-lg shadow-inner border border-[var(--color-ink)]/10 font-izhitsa leading-relaxed">
            ...
            {Object.entries(groupedSins).map(([passion, sins]) => (
              <div key={passion} className="mb-6 last:mb-0">
                <h3 className="font-izhitsa text-xl text-[var(--color-cinnabar)] border-b border-[var(--color-cinnabar)]/20 pb-1 mb-3">
                  ...
                  {sins.map(sin => {
                    ...
                    return (
                      <li key={sin.id} className="pl-4 border-l-2 border-[var(--color-ink)]/20">
                        ...
                        >
                          <span className="text-[var(--color-ink)]">{sin.title}</span>
                          ...
                          <div className="mt-2 animate-in slide-in-from-top-2 duration-200">
                            ...
                            {sinNotes[sin.id] && (
                              <div className="mt-2 bg-[var(--color-parchment)]/80 p-2 rounded text-sm border border-[var(--color-cinnabar)]/20">
                                <span className="text-[var(--color-cinnabar)] text-xs uppercase tracking-wider block mb-1">
                                  Мои обстоятельства:
                                </span>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-[var(--color-parchment)] w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 ring-1 ring-black/10">
            <div className="p-8 text-center bg-white/50">
              ...
              <h3 className="font-izhitsa text-3xl text-[var(--color-cinnabar)] mb-3">Очистить список?</h3>
              ...
              <div className="flex flex-col gap-3">
                <button 
                  ...
                  className="w-full py-4 bg-[var(--color-cinnabar)] text-[var(--color-parchment)] rounded-xl hover:brightness-110 transition-all font-izhitsa text-xl shadow-md uppercase tracking-wider"
                >
                  Да, очистить
                </button>
                <button 
                  onClick={() => setIsConfirmOpen(false)}
                  className="w-full py-3 bg-[var(--color-ink)]/5 text-[var(--color-ink)]/60 rounded-xl hover:bg-[var(--color-ink)]/10 transition-all"
                >
                  Отмена
                </button>
              </div>

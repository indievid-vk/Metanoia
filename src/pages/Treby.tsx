          ...
          <div className="space-y-12">
            {trebyData.sections.map((section) => (
              <div key={section.id} id={section.id} className="scroll-mt-8 space-y-6">
                <h2 className="font-izhitsa text-2xl text-[var(--color-cinnabar)] border-b border-[var(--color-cinnabar)]/10 pb-2">
                  {section.title}
                </h2>
                ...
                {section.content && section.content.length > 0 && (
                  <div className="space-y-4 text-[var(--color-ink)] leading-relaxed text-justify">
                    {section.content.map((paragraph, pIndex) => (
                      <div 
                        key={pIndex} 
                        className={pIndex === 0 ? "text-lg font-izhitsa italic text-[var(--color-cinnabar)]/80 mb-6 block" : ""}
                  ...
                  <div className="grid gap-6 mt-6">
                    {section.subsections.map((sub) => (
                      <div key={sub.id} id={sub.id} className="scroll-mt-8 bg-white/40 p-5 rounded-lg border border-[var(--color-ink)]/10 transition-colors hover:bg-white/60">
                        <h3 className="font-izhitsa text-xl text-[var(--color-cinnabar)] mb-3 border-b border-[var(--color-cinnabar)]/5 pb-1">
                          {sub.title}
                        </h3>
